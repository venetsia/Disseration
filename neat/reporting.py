...
class StdOutReporter(BaseReporter):
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        # Modification
        res = []
        for val in fitnesses:
            if val is "Exit":
                StdOutReporter.found_solution_checkpoint(self, config, 0, best_genome)
            elif val != None:
                res.append(val)
            else:
                res.append(0.0)
        ...
        if best_genome != 0:
            ...
            print('Population\'s average fitness: {0:3.5f} stdev: {1:3.5f}'.format(fit_mean, fit_std))
            print(
                'Best fitness: {0:3.5f} - size: {1!r} - species {2} - id {3}'.format(0,
                                                                                     0,
                                                                                     0,
                                                                                     0))
    ...
    def found_solution_checkpoint(self, config, generation, best):
        print('\nBest individual in generation {0} meets fitness threshold'.format(
            self.generation))
        return
